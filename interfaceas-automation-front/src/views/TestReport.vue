<template>
  <div class="test-report">
    <el-card class="box-card">
      <div class="title">
        【<span style="font-weight: bold">{{ testResult.taskName }}</span>】测试报告
      </div>
      <div class="summary">
      ...
      <div class="test_info">
        ...
        <el-card class="box-card"
          ...
          <el-table :data="testSuit.testCaseResults"
                    style="width: 100%"
                    size="mini">
            ...
            <el-table-column prop="testCase.name"
                             label="用例名"
                             min-width="180">
            </el-table-column>
            <el-table-column prop="inf.path"
                             label="接口地址"
                             min-width="280">
            </el-table-column>
            <el-table-column prop="inf.requestMethod"
                             label="请求方法"
                             min-width="180">
            </el-table-column>
            <el-table-column label="测试结果"
              ...
              <template slot-scope="scope">
                <el-tag effect="dark"
                        type="success"
                        v-if="scope.row.status == 0">成功</el-tag>
                <el-tag effect="dark"
                        type="warning"
                        v-if="scope.row.status == 1">失败</el-tag>
                <el-tag effect="dark"
                        type="danger"
                        v-if="scope.row.status == 2">异常</el-tag>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
      ...
      <div class="test_info">
        ...
        <el-card class="box-card"
          ...
          <div v-for="testSuit in testResult.testSuitResults"
            ...
            <div v-for="(testCaseResult, index) in testSuit.testCaseResults"
                 :key="testCaseResult.testCase.id">
              [{{ testCaseResult.startTime }}]测试用例【{{
                testCaseResult.testCase.name
              }}】测试开始...
              <pre v-if="testCaseResult.status == 0 || testCaseResult.status == 1">{{ testCaseResult.responseData }}
              </pre>
              <pre v-if="testCaseResult.status == 2">{{ testCaseResult.exception }}
              </pre>
              <div v-for="assert in testCaseResult.extracts"
                   :key="assert.assertExpression">
                [{{ testCaseResult.endTime }}]响应提取[{{
                  assert.assertExpression
                }}]，提取结果：{{ assert.result == true ? '成功' : '失败'
                }}<span v-if="assert.result == false">，实际结果：{{ assert.realValue }}。</span>
              </div>
              <div v-for="assert in testCaseResult.asserts"
                   :key="assert.assertExpression">
              ...
              <div v-for="assert in testCaseResult.dbAsserts"
                   :key="assert.assertExpression">
              ...
              [{{ testCaseResult.endTime }}]测试用例【{{
                testCaseResult.testCase.name
              }}】测试结束，测试结果：<span style="color: #67c23a; font-weight: bold"
                    v-if="testCaseResult.status == 0">成功</span><span style="color: #e6a23c; font-weight: bold"
                    v-if="testCaseResult.status == 1">失败</span>
              <span style="color: #f56c6c; font-weight: bold"
                    v-if="testCaseResult.status == 2">异常</span>
